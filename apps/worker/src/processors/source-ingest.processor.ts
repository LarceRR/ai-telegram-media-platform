...
export async function sourceIngestProcessor(job: Job, ctx: ProcessorContext): Promise<void> {
  const payload = ingestSourceJobSchema.parse(job.data);
  const service = ctx.context.get<SourcesService>('SourcesService' as never, { strict: false });
