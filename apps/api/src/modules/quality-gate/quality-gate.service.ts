...
export class QualityGateService {
  ...
  async evaluate(postId: string, actorId: string, input: QualityGateInput) {
    ...
    const row = await this.prisma.$queryRawUnsafe<Array<{ channelId: string }>>(`SELECT p.channel_id AS "channelId" FROM posts p WHERE p.id = $1 AND EXISTS (SELECT 1 FROM channel_members cm JOIN users u ON u.id = cm.user_id WHERE cm.channel_id = p.channel_id AND (u.id::text = $2 OR u.email = $2))`, postId, actorId);
    if (!row[0]) throw new AppError('FORBIDDEN', 'Post access denied');
    await this.prisma.$transaction(async (tx) => {
      if (decision === 'REVIEW' || (decision === 'WAIT' && value.mode === 'AUTO')) {
        await tx.$executeRawUnsafe(`INSERT INTO moderation_queue (post_id, channel_id, status, reason) VALUES ($1, $2, 'PENDING', $3) ON CONFLICT (post_id) DO UPDATE SET status = 'PENDING', reason = EXCLUDED.reason`, postId, row[0].channelId, `QUALITY_GATE_${decision}`);
      }
