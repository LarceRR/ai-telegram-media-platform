...
export class ChannelsService {
  ...
  async create(actorValue: string, actorName: string, dto: CreateChannelDto): Promise<ChannelResponse> {
    ...
    await this.audit.record({ actorType: 'HUMAN', actorId: user.id, action: 'channel.created', entityType: 'Channel', entityId: response.id });
    return response;
  }
  ...
  async list(actorValue: string) {
    const user = await this.actor(actorValue);
    if (!user) return [];
    const rows = await this.prisma.channel.findMany({ where: { members: { some: { userId: user.id } } }, orderBy: { createdAt: 'desc' }, include: this.include(user.id) });
    return rows.map((row) => this.toResponse(row, row.members[0]?.role ?? ChannelMemberRole.VIEWER));
  }
  ...
  async update(actorValue: string, id: string, dto: UpdateChannelDto) {
    ...
    const row = await this.prisma.channel.update({ where: { id }, data: dto as any, include: this.include(current.members[0]?.userId) });
  ...
  async updateSettings(actorValue: string, id: string, dto: UpdateSettingsDto) {
    ...
    if (role !== ChannelMemberRole.OWNER && changed.some((key) => protectedFields.has(key))) throw new ForbiddenException('Only owners can change protected settings');

    const { expectedVersion, ...settingsData } = dto;
    const result = await this.prisma.channelSettings.updateMany({ where: { channelId: id, version: expectedVersion }, data: settingsData as any });
    if (result.count !== 1) throw new ConflictException('Settings version is stale');
  ...
  async addMember(actorValue: string, id: string, dto: AddMemberDto) {
    ...
    await this.audit.record({ actorType: 'HUMAN', actorId: channel.createdById, action: 'channel.member.upserted', entityType: 'ChannelMember', entityId: channel.id });
  }

  async setCredential(actorValue: string, id: string, dto: CredentialRefDto) {
    const channel = await this.authorized(actorValue, id, ChannelMemberRole.OWNER);
    if (/token|password|api[_-]?key|bot\d*:/i.test(dto.secretRef)) throw new AppError('VALIDATION', 'Only a secret-manager reference is accepted');
    const user = await this.actor(actorValue);
    if (!user) throw new UnauthorizedException('Unknown actor');
    ...
    await this.audit.record({ actorType: 'HUMAN', actorId: user.id, action: 'channel.credential_reference.updated', entityType: 'SourceCredentialRef', entityId: channel.id });
  ...
  private async authorized(actorValue: string, id: string, minimum: ChannelMemberRole) {
    const user = await this.actor(actorValue);
    const row = user ? await this.prisma.channel.findFirst({ where: { id, members: { some: { userId: user.id } } }, include: this.include(user.id) }) : null;
    if (!row) throw new ForbiddenException('Channel access denied');
    const role = row.members[0]?.role;
    if (!role || rank[role] < rank[minimum]) throw new ForbiddenException('Insufficient channel permissions');
    return row;
  }

  private toResponse(channel: any, role: ChannelMemberRole): ChannelResponse & { version: number } {
