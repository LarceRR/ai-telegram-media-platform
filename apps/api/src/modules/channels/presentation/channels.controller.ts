...
export class ChannelsController {
  ...
  private actor(headers: Record<string, unknown>): string { const value = headers['x-actor-id'] ?? headers['x-user-id']; if (typeof value !== 'string' || value.length < 1) throw new Error('actor header is required'); return value; }
  @Post() create(@Headers() h: Record<string, unknown>, @Body() dto: CreateChannelDto) { return this.channels.create(this.actor(h), String(h['x-user-name'] ?? this.actor(h)), dto); }
  ...
  @Put(':id/telegram-credential') credential(@Headers() h: Record<string, unknown>, @Param('id') id: string, @Body() dto: CredentialRefDto) { return this.channels.setCredential(this.actor(h), id, dto); }
}
